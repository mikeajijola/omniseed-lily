...
The Engine resumes that work by sending an authenticated `POST /stewardship/tick` to the runtime. The runtime resolves the bearer secret indirectly through `OMNISEED_STEWARDSHIP_TRIGGER_CREDENTIAL_ENV`, ignores caller-supplied workflow state, and fetches the current profile, limits, queue, repairs, and durable session identifiers from `get_stewardship_status` before every tick. Each selected item advances by at most one governed operation, and independent items run concurrently only up to the profile's declared bound.
