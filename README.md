...
Requests for autonomous or time-bounded stewardship use that same contract. Lily discovers any active stewardship profile and its limits from governed company state, then may inspect, observe, plan, and submit exact proposals within her existing authority. Conversational labels such as “YOLO mode” do not grant authority or activate a profile: status, enablement, pause, and disablement become authenticated Engine operation requests. Durable steps retain the Engine-issued session identifier and request submission, independently reviewed merge evaluation, reconciliation, and observation one operation at a time. Expiry, kill-switch state, protected-change handling, independent approval, checks, and concurrency remain Engine-governed; Lily stops when one of those boundaries requires another actor or current evidence no longer establishes that the profile is active.

The authenticated `POST /stewardship/tick` endpoint is a scheduler integration boundary, not proof that the published Engine supplies a durable stewardship queue. Status uses the Engine's actual `inspect_stewardship` operation and its `limits.concurrency` / `usage.active` fields. Disabled, paused, expired, and undeclared profiles stop honestly; an enabled profile without a governed queue and atomic claim contract returns `stewardship_scheduler_unavailable`. Request-only transitions are invoked only when current company inspection declares an implemented, available handler. They never fall back to direct apply, merge, or approval. The remaining Engine queue/claim/request integration and live continuation evidence are required before #17 can be completed.

Company inspection includes the Engine-returned desired intent and outcome descriptions so prioritisation can use company goals as well as gaps. These facts are read dynamically and are not embedded in instructions.
