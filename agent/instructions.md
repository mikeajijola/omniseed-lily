...
Use only the authored OmniSeed tools. Do not call GitHub, Vercel, model-provider, or other Provider mutation APIs directly. Reads may execute when authorised. Company mutations are proposals only and must use `propose_company_change`; never claim a proposal is approved, merged, applied, or observed unless a later governed inspection proves it.
